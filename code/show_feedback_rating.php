...
<body>
    <div class="container">
    	<h1 class="mt-5 mb-5" style="color: white" >Welches Feedback hast du für uns?</h1>
    	<div class="card">
    		<div class="card-header" style="font-size:20px;" >So habt ihr uns bewertet: </div>
    		<div class="card-body">
    			<div class="row">
...
$('#save_review').click(function(){
    ...
    {
        alert("Füllen Sie bitte beide Felder aus!"); //error-alert if at least one textfield is empty 
        return false;
    }
    else
    {
        ...
        $.ajax({
            url:"feedback_submit_rating.php", //a string containing the URL to which the request is sent.
            method:"POST", //HTTP method to use for the request
            data:{rating_data:rating_data, user_name:user_name, user_review:user_review}, //data to be sent to the server 
        ...
        success:function(data)
        {
            ...
            if(data.review_data.length > 0)
            {
                ...
                for(var count = 0; count < data.review_data.length; count++)
                {
                    ...
                    html += '<br />';
                    html += 'Dein Feedback an uns:'; 
                    html += '<br />';

                    html += data.review_data[count].user_review;

                    if(data.review_data[count].rating > 3){
                        html += '<br />';
                        html += '<br />';
                        html += 'Wasgehtab:';
                        html += '<br />';
                        html += 'Vielen Dank für das nette Feedback!'; 
                    }
                    else
                    {
                        html += '<br />';
                        ...
                        html += 'Wasgehtab:';
                        html += '<br />';
                        html += 'Schade, dass deine Erfahrungen so gelaufen sind.'
                        html += '<br />';
                        html += 'Wir werden daran arbeiten, die angesprochenen Punkte zu verbessern!'; 
                    }
